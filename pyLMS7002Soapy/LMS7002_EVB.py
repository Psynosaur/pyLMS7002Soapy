...
from pyLMS7002Soapy.ADF4002 import ADF4002
from pyLMS7002Soapy.Si5351 import Si5351
...
class LMS7002_EVB(object):
    ...
    def __init__(self, portName='/dev/ttyACM0', fRef=30.72e6, verbose=0):
        ...
        if not self.ser.isOpen():
            self.ser = None
            raise IOError(
                "Could not open port " + portName + "\nPlease ensure that the port exists and that the permissions "
                                                    "are set correctly.\nOn Linux systems permissions can be set by "
        ...
        self.LMS7002 = LMS7002(SPIwriteFn=self.LMS7002_Write, SPIreadFn=self.LMS7002_Read, verbose=verbose,
                               MCUProgram=self.MCUProgram, fRef=self.fRef)
